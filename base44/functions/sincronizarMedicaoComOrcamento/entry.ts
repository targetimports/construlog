...
// Sincroniza dados do orçamento para a medição
Deno.serve(async (req) => {
  try {
    ...
    const itensMedicao = await base44.asServiceRole.entities.ItemMedicao.filter({
    ...
    for (const itemOrc of itensOrcamento) {
      ...
      const dadosOrcamento = {
        ...
        valor_unitario: itemOrc.preco_unitario_final || itemOrc.custo_unitario_calculado,
        bdi_aplicado: itemOrc.bdi_aplicado,
        custo_unitario: itemOrc.custo_unitario_calculado,
        origem_preco: itemOrc.origem_preco
      };

      if (itemMedicaoExistente) {
        // Atualizar item existente
        promises.push(
          base44.asServiceRole.entities.ItemMedicao.update(itemMedicaoExistente.id, dadosOrcamento)
        );
      } else {
        // Criar novo item de medição
        promises.push(
          base44.asServiceRole.entities.ItemMedicao.create({
            medicao_id: medicao_id,
            ...dadosOrcamento,
            quantidade_prevista: itemOrc.quantidade,
    ...
    return Response.json({ 
      success: true,
      message: 'Medição sincronizada com orçamento',
      itens_atualizados: promises.length,
      orcamento_id: orcamento.id
    });
  } catch (error) {
    console.error('Erro ao sincronizar medição:', error);
